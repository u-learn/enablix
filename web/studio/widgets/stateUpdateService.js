	...
	[
	 	...
	 	function($state,   $stateParams,   LocationUtil,   $location,   $window,   $transitions,   NavigationTracker,   TenantInfoService,   ActivityTrackerContext,   NextStateUrlParams,   CacheService) {
			...
			var goToForgotPasswordPage = function(){
	 		...
	 		var goToPortalContainerList = function(_containerQId, _layout, _urlParams, _searchQuery) {
	 		...
	 		var goToPortalEnclosureDetail = function(_enclosureId, _childContainerId) {
	 			...
	 			$state.go("portal.enclosure", {
	 				...
	 				"childContainerQId": (_childContainerId ? _childContainerId : null)
	 			});
	 		}
	 		...
	 		var goToMyDraftContentDetail = function(refObjectIdentity) {
	 			$state.go("myaccount.contentdraftview", {
	 				"action": "view",
	 				"refObjectIdentity": refObjectIdentity
	 			});
	 		}
