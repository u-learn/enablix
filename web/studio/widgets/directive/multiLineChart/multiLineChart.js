		...
		return {
			...
			link: function(scope, element, attrs) {
				...
				function flushChartArea() {
					...
					d3.select("#multi-chart").select("#innerBoxDiv").remove();
				...
				function makeLineChart(dataset, xName, yObjs, axisLables) {
					...
					chartObj.bind = function (selector) {
						...
						chartObj.mainDiv.append("div").attr("class", "inner-wrapper").attr("id","innerWrapperDiv").append("div").attr("class", "outer-box").attr("id","outerBoxDiv").append("div").attr("class", "inner-box").attr("id","innerBoxDiv");
					...
					chartObj.render = function () {
						...
						function mousemove() {
							...
							for (var y  in yObjs) {
								if(isNaN(chartObj.yScale(yObjs[y].yFunct(d))) || isNaN(chartObj.xScale(chartObj.xFunct(d)))) {
									continue;
								}
