...
public class TrendUtil {
	public static List<MetricStats> updateDailyMissingValues(List<TrendAggMetric> trendAggMetricLst, Date startDate, Date endDate, String metricCode, 
			String metricName) {
        ...
        while( startDate.compareTo(endDate)<= 0 ) {
        	...
        	if(startDateDimension.getDayOfYear()==trendAggMetric.getDayOfYear()) {
            	metricStat = new MetricStats(metricCode, metricName, trendAggMetric.getValue());
            	metricStatLst.add(metricStat);
            	startDate = DateUtil.getNextDate(startDate);
        	}
        	else {
        		metricStat = new MetricStats(metricCode, metricName, new Long(0));
        		metricStatLst.add(metricStat);
        		startDate = DateUtil.getNextDate(startDate);
        		continue;
        	}
	...
	public static List<MetricStats> updateWeeklyMissingValues(List<TrendAggMetric> trendAggMetricLst, Date startDate, Date endDate, String metricCode, 
			String metricName) {
		List<MetricStats> metricStatLst = new ArrayList<MetricStats>();
        int diffCounter = 0;
	...
	public static List<MetricStats> updateMonthlyMissingValues(List<TrendAggMetric> trendAggMetricLst, Date startDate,
