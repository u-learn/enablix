...
public class SubContainerCoverageResolver {
	...
	public long getCount(ContainerType subContainer, ContentDataRecord record, TemplateWrapper template) {
		...
		if (TemplateUtil.hasOwnCollection(template.getContainerDefinition(contentQId ))) {
			...
			count = crud.findRecordCountWithParentId(collName, recordIdentity);
			...
			count  = crud.findChildElementsCount(collName, qIdRelativeToParent, recordIdentity);
		}
